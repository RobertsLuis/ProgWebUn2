...
document.addEventListener("DOMContentLoaded", function () {
    ...
    if (dadosArquivados) {
        document.getElementById('dropbox').value = dadosArquivados;
        modificarTurma();
    }
});
...
abreNovo = function() {
    sessionStorage.setItem('turmaSelecionada', document.getElementById("dropbox").value);
    window.location.href = '/novo';
}

function sysout(mensagem){
    fetch('/log', {
        method: 'POST',
        headers: {
...
function verificarMatricula(matricula){
    let request = new XMLHttpRequest();
    request.open('GET', '/getByMatricula?matricula=' + encodeURIComponent(matricula), false);
    request.send(null);
...
function validarEnvio(dto) {
    ...
    result['turma'] = dto.turma === "1" || dto.turma === "2";
    result['nome'] = dto.nome.length >= 5;
    result['matricula'] = dto.matricula.length >= 6 && dto.matricula.length <= 10;
    result['nota'] = parseFloat(dto.nota) >= 0 && parseFloat(dto.nota) <= 10;

    if (dto.id === ''){
        result['matricula'] = (result['matricula'] && (!verificarMatricula(dto.matricula)))
    }

    let resultSummary = result['turma'] && result['nome'] && result['matricula'] && result['nota']
    return {"status": resultSummary, "details": result};
